...
var fetchUsers = function (text) {
  ...
  var results = text.match(/@[a-zA-Z0-9_\u4e00-\u9fa5\-_]+/igm);
  var names = [];
  if (results) {
 ...
 */
exports.sendMessageToMentionUsers = function (text, talk_id, authorId, reply_id, callback) {
  if (typeof reply_id === 'function' || !reply_id) {
    callback = reply_id;
    reply_id = null;
  }
  ...
  User.getUsersByNames(fetchUsers(text), function (err, users) {
    ...
    if(!reply_id){
      users.forEach(function (user) {
        Message.sendAtMessage(user._id, authorId, talk_id, reply_id, ep.done('sent'));
      });
    }else{
      console.log("a");
      users.forEach(function (user) {
        Message.sendReply2Message(user._id, authorId, talk_id, reply_id, ep.done('sent'));
      });
    }
  });
};
